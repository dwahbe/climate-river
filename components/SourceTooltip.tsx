    ...
    <>
      ...
      {isVisible && (
        <div
          ...
          className="fixed z-50 bg-white rounded-control shadow-sm border border-zinc-200 p-3 max-w-[min(calc(100vw-32px),24rem)] w-auto min-w-60 sm:min-w-64"
          style={{
            left: position.x,
            top: position.y,
            ...
            <div className="space-y-3">
              {articles.map((article) => (
                <div key={article.article_id} className="space-y-1">
                  <a
                    href={article.url}
                    ping={`/api/click?aid=${article.article_id}`}
